...
void run_bench(const igraph_t *graph, const igraph_vector_t *weights,
    ...
    BENCH(msg, REPEAT(igraph_community_leiden(graph, weights, &vertex_weight, 1.0 / igraph_vector_sum(weights), 0.01, 1, false, 1, true, false, &membership, NULL, NULL, NULL), rep));
